# DEV-WORKFLOW.md — Guide de développement & Infrastructure
...
16. [Bundle Analyzer](#16-bundle-analyzer)
17. [Dev Container](#17-dev-container)
18. [Releases & Changelog](#18-releases--changelog)
19. [Intégration IA](#19-intégration-ia)
...
│   │   ├── layout.tsx                # Layout (metadata, PWA, analytics)
...
├── .cursorrules                      # Contexte Cursor AI
├── DEV-WORKFLOW.md                   # CE FICHIER
├── Makefile                          # Automatisation (push, deploy…)
...
npm run test:coverage # Couverture de code (v8)
```

Les tests se trouvent dans `src/__tests__/` et suivent la structure source :
...
```bash
npx playwright test             # Exécuter les tests E2E
npx playwright test --ui        # Mode interactif
```

Les tests E2E sont dans `e2e/` et testent le build statique servi localement.
...
Pour visualiser la taille du bundle JavaScript :
...
Cela permet de détecter les dépendances lourdes et optimiser la taille du build.
...
Pour l'utiliser : ouvrir le projet dans VS Code → **Dev Containers: Reopen in Container**.

---

## 18. Releases & Changelog

### Changelog

Générer le changelog depuis les conventional commits :

```bash
npm run changelog
# Crée / met à jour CHANGELOG.md
```

### Releases automatiques

Créer et taguer une release :
...
Le workflow `release.yml` crée automatiquement une **GitHub Release** avec des notes regroupées par type (features, fixes, infra, docs).
